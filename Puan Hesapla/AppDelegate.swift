...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplication.LaunchOptionsKey: Any]?) -> Bool {
        ...
        if let url = launchOptions?[.url] as? URL {
            ...
            return DeepLinkManager().execute(with: url).response
        }else{
            return true
        }
    }
    func application(_: UIApplication, open url: URL, options: [UIApplication.OpenURLOptionsKey : Any] = [:]) -> Bool {
        window?.rootViewController = DeepLinkManager().execute(with: url).vc
        window?.rootViewController!.present(DeepLinkManager().execute(with: url).web, animated: true)
        return url.scheme == "app" && DeepLinkManager().execute(with: url).response
    }
    ...
    lazy var persistentContainer: NSPersistentContainer = {
        ...
        */
        let container = NSPersistentContainer(name: "Puan_Hesapla")
